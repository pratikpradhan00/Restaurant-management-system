		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Connection sos = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3307/rms","root","");
					
					String sql = "Select * from logindatabase where username=? and password=?";
					PreparedStatement pst = (PreparedStatement) sos.prepareStatement(sql);
					pst.setString(1, textField.getText());
					pst.setString(2, passwordField.getText());

					ResultSet rs = (ResultSet) pst.executeQuery();
					if(rs.next()) {
						JOptionPane.showMessageDialog(null, "Username and password are matching");
						After_login menu = new After_login();
